    ...
    <div @click.stop=""  class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
        <div  v-if="type!='time'">
            <div class="calendar-tools">
                <span title="上一月" class="calendar-prev  icon-l-play" @click="prev"></span>
                <span title="下一月" class="calendar-next icon-r-play"  @click="next"></span>
                <div class="text center">
                    <input type="number" v-model="year" value="{{year}}" @change="render(year,month)" min="1970" max="2100" maxlength="4">年
                    {{monthString}}&nbsp月
                </div>
            </div>
            <table cellpadding="5">
...
export default {
    ...
    methods: {
        ...
        ok() {
            ...
            if (vm.range) {
                vm.value = vm.output(vm.rangeBegin) + " ~ " + vm.output(vm.rangeEnd)
            } else {
                vm.value = vm.output([
                    vm.year,
                    vm.month, 
            ...
            vm.show = false;
            // 派发时间
            vm.$dispatch('filterLog',vm.value);
        },
        // 隐藏控件
        cancel() {
            this.show = false
