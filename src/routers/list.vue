                   ...
                   <div class="e-doc">
                       ...
                       <div class="e-mov-scroll scroll">
                           <div class="model-item" v-for="node in nodes">
                               ...
                               <span :title="node.Title">{{node.Section}}&nbsp{{node.Title}}&nbsp{{node.PageFrom}}-{{node.PageEnd}}</span>
                           </div>
                       </div>
                   </div>
                   <div class="e-detail">
    ...
    module.exports = {
        ...
        methods: {
           ...
           checklegal:function(tag){
            ...
            getNodes:function () {
                let vm=this;
                let resource = vm.$resource('content/getByN?name='+encodeURI(vm.curDoc)+'&type='+vm.curType);
                resource.get()
                    .then((response) => {
                        if(response.data.Code==200){
            ...
            // 退出评审
            backto:function(){
                this.$router.go({path:'./'+this.stage});
            }
        },
        events:{
